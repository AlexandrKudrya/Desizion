...
interface AlternativeDialogContentProps {
  alternative: Alternative | null
  criteria: Criterion[]
  onClose: () => void
  onSave: (data: Omit<Alternative, 'id'>) => void
...
function AlternativeDialogContent({ alternative, criteria, onClose, onSave }: AlternativeDialogContentProps) {
  const [name, setName] = useState(alternative?.name ?? '')
  const [description, setDescription] = useState(alternative?.description ?? '')
  const [values, setValues] = useState<Record<string, number | string | null>>(alternative?.values ?? {})
