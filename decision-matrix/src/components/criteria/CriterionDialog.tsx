...
interface CriterionDialogContentProps {
  criterion: Criterion | null
  onClose: () => void
  onSave: (data: Omit<Criterion, 'id'>) => void
}

function CriterionDialogContent({ criterion, onClose, onSave }: CriterionDialogContentProps) {
  const [name, setName] = useState(criterion?.name ?? '')
  const [weight, setWeight] = useState(criterion ? Math.round(criterion.weight * 100) : 10)
  const [normType, setNormType] = useState<NormalizationType>(criterion?.normType ?? NormalizationType.LINEAR)
  const [params, setParams] = useState<NormalizationParams>(criterion?.params ?? defaultParams[NormalizationType.LINEAR])
